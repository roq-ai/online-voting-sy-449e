...
import { useRoqClient, useCandidateFindFirst } from 'lib/roq';
...
function CandidateEditPage() {
  ...
  const { data, error, isLoading, mutate } = useCandidateFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CandidateInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.candidate.update({
        data: values as RoqTypes.candidate,
        where: {
          id,
        },
      });
      ...
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <NumberInput
            ...
            formControlProps={{
              id: 'votes',
              isInvalid: !!formik.errors?.votes,
            }}
            name="votes"
            error={formik.errors?.votes}
            value={formik.values?.votes}
            onChange={(valueString, valueNumber) =>
              formik.setFieldValue('votes', Number.isNaN(valueNumber) ? 0 : valueNumber)
            }
          />
          ...
          <AsyncSelect<ElectionInterface>
            ...
            fetcher={() => roqClient.election.findManyWithCount({})}
            labelField={'title'}
          />
