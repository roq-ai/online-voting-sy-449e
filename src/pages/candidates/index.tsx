...
import { useRoqClient, useCandidateFindManyWithCount } from 'lib/roq';
...
export function CandidateListPage(props: CandidateListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useCandidateFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.candidate.delete({ where: { id } });
      await mutate();
    } catch (error) {
    ...
    hasAccess('election', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/elections/view/${record.election?.id}`}>
