...
import { useRoqPlatformUserProfiles, useCandidateCount, useVoteCount, useAuditLogCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const candidateStat = useCandidateCount();

  const voteStat = useVoteCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Candidates',
      result: candidateStat?.data,
      isLoading: candidateStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Votes',
      result: voteStat?.data,
