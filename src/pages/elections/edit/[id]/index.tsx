...
import { useRoqClient, useElectionFindFirst } from 'lib/roq';
...
function ElectionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useElectionFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ElectionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.election.update({
        data: values as RoqTypes.election,
        where: {
          id,
        },
      });
