...
import { useRoqClient, useVoteFindFirst } from 'lib/roq';
...
function VoteEditPage() {
  ...
  const { data, error, isLoading, mutate } = useVoteFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: VoteInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.vote.update({
        data: values as RoqTypes.vote,
        where: {
          id,
        },
      });
      ...
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<CandidateInterface>
            ...
            fetcher={() => roqClient.candidate.findManyWithCount({})}
            labelField={'name'}
          />
